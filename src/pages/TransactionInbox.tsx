...
import { Check, X, Edit2, AlertCircle, Inbox, Sparkles, Filter } from 'lucide-react';
                        ...
                        <div className="overflow-x-auto">
                            <table className="w-full text-left">
                                <thead className="bg-surface-highlight/30 border-b border-border">
                                    <tr>
                                        <th className="px-6 py-4 text-xs font-semibold text-text-muted uppercase tracking-wider">日付</th>
                                        <th className="px-6 py-4 text-xs font-semibold text-text-muted uppercase tracking-wider">説明・項目</th>
                                        <th className="px-6 py-4 text-xs font-semibold text-text-muted uppercase tracking-wider">カテゴリ</th>
                                        <th className="px-6 py-4 text-xs font-semibold text-text-muted uppercase tracking-wider text-right">金額</th>
                                        <th className="px-6 py-4 text-xs font-semibold text-text-muted uppercase tracking-wider text-center">操作</th>
                                    </tr>
                                </thead>
                                    ...
                                    {pendingTransactions.map((t) => (
                                        <tr key={t.id} className="hover:bg-surface-highlight/20 transition-colors group">
                                            ...
                                            <td className="px-6 py-4">
                                                <div className="font-medium text-text-main">{t.item}</div>
                                                <div className="text-xs text-text-muted truncate max-w-[200px]">{t.description}</div>
                                            </td>
                                            <td className="px-6 py-4 text-sm">
                                                <span className="px-2 py-1 bg-surface-highlight rounded text-text-secondary border border-border">
                                                    {t.category}
                                                </span>
                                            </td>
                                            ...
                                            <td className="px-6 py-4">
                                                <div className="flex items-center justify-center gap-2">
                                                    <button
                                                        ...
                                                        className="p-2 bg-emerald-500/10 text-emerald-500 rounded-lg hover:bg-emerald-500 hover:text-white transition-all shadow-sm flex items-center gap-1 text-xs"
                                                        title="承認する"
                                                    >
                                                        <Check className="w-4 h-4" />
                                                        承認
                                                    ...
                                                    <button
                                                        ...
                                                        disabled={!!processingId}
                                                        className="p-2 bg-rose-500/10 text-rose-500 rounded-lg hover:bg-rose-500 hover:text-white transition-all shadow-sm flex items-center gap-1 text-xs"
                                                        title="削除する"
                                                    >
                                                        <X className="w-4 h-4" />
                                                        削除
