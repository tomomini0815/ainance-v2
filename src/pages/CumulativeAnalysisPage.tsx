            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
                    <div className="bg-white dark:bg-surface p-5 rounded-xl border border-border shadow-sm">
                        <div className="text-xs text-text-muted mb-1">累計予算合計</div>
                        <div className="text-2xl font-bold text-text-main">{analysis.cumulativeBudget.toLocaleString()}円</div>
                        <div className="mt-2 text-xs text-text-muted flex items-center gap-1">
                            <Target className="w-3 h-3" /> {analysis.monthCount}ヶ月分
                        </div>
                    </div>
                    <div className="bg-white dark:bg-surface p-5 rounded-xl border border-border shadow-sm">
                        <div className="text-xs text-text-muted mb-1">累計支出実績</div>
                        <div className="text-2xl font-bold text-emerald-500">{analysis.totalExpense.toLocaleString()}円</div>
                        <div className="mt-2 text-xs text-emerald-500/70 font-medium">
                            消化率 {analysis.progress.toFixed(1)}%
                        </div>
                    </div>
                    <div className="bg-white dark:bg-surface p-5 rounded-xl border border-border shadow-sm">
                        <div className="text-xs text-text-muted mb-1">累計予算残高</div>
                    ...
                    <div className="bg-white dark:bg-surface p-5 rounded-xl border border-border shadow-sm">
                        <div className="text-xs text-text-muted mb-1">総合状況</div>
                        {analysis.variance >= 0 ? (
                            <div className="flex items-center gap-2 text-green-500 mt-1">
                                <TrendingUp className="w-8 h-8" />
                                <span className="text-lg font-bold">予算内</span>
                            </div>
                        ) : (
                            <div className="flex items-center gap-2 text-red-500 mt-1">
                                <AlertCircle className="w-8 h-8" />
                                <span className="text-lg font-bold">警告</span>
                            </div>
                        )}
                    </div>
                </div>
