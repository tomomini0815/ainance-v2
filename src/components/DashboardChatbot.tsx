...
const DashboardChatbot: React.FC = () => {
    ...
    const handleSubmit = async (e?: React.FormEvent) => {
        ...
        try {
            ...
            if (aiData) {
                ...
                let amount = 0;
                const amountTextMatch = userMessage.match(/((?:\d+[,\s]?)+(?:万(?:\s?\d+)*)?(?:千(?:\s?\d+)*)?円?)/);
                if (amountTextMatch) {
    ...
    return (
        <div className={`fixed bottom-24 md:bottom-32 right-6 z-40 flex flex-col items-end transition-all duration-300 pointer-events-none`}>
            {/* Chat Window */}
            {isOpen && (
                <div className="relative bg-white dark:bg-surface border border-border shadow-2xl rounded-2xl w-80 sm:w-96 h-[500px] mb-4 flex flex-col overflow-visible animate-in slide-in-from-bottom-5 duration-200 pointer-events-auto">
                    {/* Tail */}
            ...
            <button
                ...
                className={`flex items-center justify-center gap-2 px-5 py-4 rounded-full shadow-lg shadow-primary/25 transition-all duration-300 hover:scale-105 active:scale-95 pointer-events-auto ${isOpen
