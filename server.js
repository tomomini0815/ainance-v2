...
app.post('/api/update-coordinates', async (req, res) => {
  try {
    ...
    const targetFile = 'src/services/pdfDigitBoxService.ts';
    const fs = await import('fs/promises');

    let content = await fs.readFile(targetFile, 'utf8');

    // Find the start and end of the calibration section
    const startMarker = '// ===== COORDINATE CONFIGURATION =====';
    const endMarker = '// ===== CALIBRATION OFFSETS =====';

    const startIndex = content.indexOf(startMarker);
    const endIndex = content.indexOf(endMarker);

    if (startIndex === -1 || endIndex === -1) {
      return res.status(500).json({ error: 'Could not find coordinate configuration section in target file' });
    }

    // Replace the content
    const newContent = content.slice(0, startIndex) + code + '\n' + content.slice(endIndex);
    ...
    res.status(500).json({ error: 'Failed to update coordinates: ' + error.message });
  }
});
